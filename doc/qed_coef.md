...
leading to Eq. (B6) in [[1]](#1) if reduced further. An interesting observation here is that the sum of $S(\mathbf{n},\mathbf{v})$ on $\mathbf{n}$ has the same UV (i.e. large $|\mathbf{n}|$) behaviour than the integral, and therefore
